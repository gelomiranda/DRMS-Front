    ...
    <CRow>
      <CCol sm="12">
        <CCard>
          ...
          <CButton type="submit" size="sm" color="primary" :block="true" @click.prevent="update">
...
export default {
  ...
  created(){
    /**
     * Reset the state visibility
     * Fetch the patient by id
     * Fetch the nephrologist
     * */
    this.$store.state.alertVisibility = false;
    api.get('patients/' + this.$route.params.id)
    .then((response) => {
  ...
  methods: {
    update () {
        api.update('patients/',this.$route.params.id, this.form )
        .then((response) => {
